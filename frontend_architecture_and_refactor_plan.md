...
阶段二的 `InstructionCatalog` 及 Python/C++ 共用语义表已经建立，Python canonical builder 也已提供。受限版 `CoreLoweringPass` 已能执行无显式 dependency、无 loop-carried、`unroll=1`、默认 induction 的 canonical 程序，并在不支持的动态语义上明确报错。当前 CCE binder 仍输出迁移期 `VFInfo`；下一步是实现结构化动态 iteration/value identity，先支持 `unroll=1` 的 loop-carried，再扩展 `unroll>1`，同时让 CCE/legacy adapter 直接生成 canonical 对象。待 canonical 主路径覆盖现有输入后，再删除旧 normalize/specialize 逻辑；迁移期间不新增第二套隐式转换。
