...
constexpr size_t kRepeatTimes = 16;
...
void BuildInput0(std::vector<float> &input0)
{
...
void BuildInput1(std::vector<float> &input1)
{
    for (size_t i = 0; i < input1.size(); ++i) {
        input1[i] = static_cast<float>((static_cast<int>(i % 17) - 8) * 0.0625f);
    }
}
...
void GeluPolyReference(const std::vector<float> &input0, std::vector<float> &golden)
{
...
bool BuildGolden(const std::string &kernelName, const std::vector<float> &input0, std::vector<float> &golden)
{
    if (kernelName == "foo_add") {
        GeluPolyReference(input0, golden);
        return true;
    }
    return false;
}
...
int main(int argc, char **argv)
{
    ...
    RT_CHECK(rtMalloc(&input1Dev, kBytes, RT_MEMORY_HBM, 0));
    ...
    RT_CHECK(rtMemcpy(input1Dev, kBytes, input1Host.data(), kBytes, RT_MEMCPY_HOST_TO_DEVICE));
    ...
    (void)WriteFloatFile("input1.bin", input1Host);
...
cleanup:
    ...
    if (input1Dev != nullptr) {
        (void)rtFree(input1Dev);
    }
