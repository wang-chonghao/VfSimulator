...
LOG=$APPROOT/gdb_connect_disas.log
CMDS=$APPROOT/gdb_connect_disas.cmds
cat > "$CMDS" <<'EOF'
...
disas _ZN17totem_v2_chi_ring10TMultiRing17connectRingBridgeEPNS_12TConnectNodeEb
quit
EOF
gdb -q -batch -x "$CMDS" > "$LOG" 2>&1 || true
python3 - <<'PY'
from pathlib import Path
p = Path('/mnt/d/VfSimulator/ascend_runner/build/src_fanout_probe_cce_min/gdb_connect_disas.log')
