...
gdb -q -batch \
  ...
  -ex 'break totem_v2_chi_ring::TMultiRing::readConfigFile(std::basic_string<char, std::char_traits<char>, std::allocator<char> >)' \
  -ex run \
  -ex 'tbreak *($pc+0x78a6)' \
  -ex continue \
  -ex 'print/x $pc' \
  ...
  -ex 'print/x $r13' \
  -ex 'x/12gx $rsp+0x40' \
  -ex 'x/s *(char**)($rsp+0x50)' \
  -ex 'x/s *(char**)($rsp+0x58)' \
  -ex 'bt 8' \
  --args "$APP" > "$LOG" 2>&1 || true
sed -n '1,260p' "$LOG"
