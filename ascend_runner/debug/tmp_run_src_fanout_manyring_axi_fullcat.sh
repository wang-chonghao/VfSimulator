...
LOG=$APPROOT/run_manyring_axi_fullcat.log
...
for so in libshim_cfg_manyring.so libshim_axi_ctor.so libshim_cfg_axi_abi0.so; do
  cp -f "$SRC_SHIM_DIR/$so" "$APPROOT/"
done
cp -f /mnt/d/VfSimulator/ascend_runner/build/src_fanout_probe_cce_min/libshim_catDiePortId.so "$APPROOT/"
cp -f /mnt/d/VfSimulator/ascend_runner/build/src_fanout_probe_cce_min/libshim_log_catDiePortId.so "$APPROOT/"
...
cat > "$APPROOT/etc/1982_cloud_config.toml" <<'EOF'
[ARCH]
cube_core_num = 1
vec_core_num = 2
