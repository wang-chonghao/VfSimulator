...
RESULTS_ROOT = ROOT / "results" / "VADDS64_formula_compare_satk_log2i_a25"
...
def plot_trip(trip_dir: Path, rows, trip_count: int):
    ...
    plt.plot(xs, frm, marker="s", linewidth=2.5, markersize=7, linestyle="--", label="Saturated Formula (7+2.5log2I)")
    ...
    plt.tight_layout()
    plt.savefig(trip_dir / f"u_curve_compare_i{trip_count}_satk_log2i_a25.png", dpi=220)
    plt.close()
...
def plot_overview(all_rows):
    ...
    for ax, trip_count in zip(axes.flat, show_is):
        ...
        ax.plot(xs, frm, marker="s", linewidth=2.2, markersize=6, linestyle="--", label="Saturated Formula (7+2.5log2I)")
    ...
    fig.suptitle("64-VADDS Chain: Saturated Formula(7+2.5log2I) vs Simulator", fontsize=18)
    plt.tight_layout()
    plt.savefig(RESULTS_ROOT / "u_curve_compare_overview_satk_log2i_a25.png", dpi=220)
    plt.close()
...
def main():
    ...
    for trip_count in TRIP_COUNTS:
        ...
        dump_json(trip_dir / "compare_results_satk_log2i_a25.json", rows)
