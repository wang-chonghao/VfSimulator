...
def parse_cce_vf_window(workdir: str):
    base = Path(workdir)
    instr = base / "core0.veccore0.instr_log.dump"
    popped = base / "core0.veccore0.instr_popped_log.dump"
    pats = ("RV_VLOOPv2", "RV_VLD", "RV_VMULS", "RV_VEXP", "RV_VADDS", "RV_VST")
    ...
    return popped_first, instr_last, instr_last - popped_first


def write_trace(case_dir: Path, I: int, U: int, body):
    obj = {
        "dtype": "fp32",
        "params": {"I": I, "U": U},
        "program": [{"type": "loop", "iters": "I", "unroll": "U", "body": body}],
    }
...
def write_dsl(case_dir: Path, stem: str, I: int, U: int, body_lines):
...
def run_case(case, I: int, U: int):
    stem = f"{case['name']}_i{I}_u{U}"
    case_dir = ROOT / "results" / "micro_cases" / stem
    case_dir.mkdir(parents=True, exist_ok=True)

    trace = write_trace(case_dir, I, U, case["body"])
    dsl = write_dsl(case_dir, stem, I, U, case["dsl_body"])
    ...
    workdir = f"/home/lenovo/msprof_run/{stem}_native_simexec"
    _, _, cce_vf = parse_cce_vf_window(workdir)
