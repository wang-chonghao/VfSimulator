...
RESULTS_ROOT = ROOT / "results" / "VADDS64_formula_compare_powerk_sync9"
...
def weff(n: int) -> float:
    return 1.0 + K * ((N0 / float(n)) ** N1)


def formula_score(trip_count: int, loops: int, total_vadds: int = 64) -> float:
    n = total_vadds // loops
    c = SHORT_FWD * n
    segments = loops
    return (trip_count * c / weff(n) + SYNC_COST) * segments
...
def plot_trip(trip_dir: Path, rows, trip_count: int):
    ...
    plt.plot(xs, frm, marker="s", linewidth=2.5, markersize=7, linestyle="--", label="Power-k Formula (sync=9)")
    ...
    plt.tight_layout()
    plt.savefig(trip_dir / f"u_curve_compare_i{trip_count}_powerk_sync9.png", dpi=220)
    plt.close()
...
def plot_overview(all_rows):
    ...
    for ax, trip_count in zip(axes.flat, show_is):
        ...
        ax.plot(xs, frm, marker="s", linewidth=2.2, markersize=6, linestyle="--", label="Power-k Formula (sync=9)")
    ...
    fig.suptitle("64-VADDS Chain: Power-k(sync=9) Formula vs Simulator", fontsize=18)
    plt.tight_layout()
    plt.savefig(RESULTS_ROOT / "u_curve_compare_overview_powerk_sync9.png", dpi=220)
    plt.close()
...
def main():
    ...
    for trip_count in TRIP_COUNTS:
        ...
        src_rows = json.loads((SRC_ROOT / f"I{trip_count}" / "compare_results.json").read_text(encoding="utf-8"))

        rows = []
        for r in src_rows:
            loops = int(r["loops"])
            vadds_per_loop = int(r["vadds_per_loop"])
            rows.append({
                "loops": loops,
        ...
        dump_json(trip_dir / "compare_results_powerk_sync9.json", rows)
