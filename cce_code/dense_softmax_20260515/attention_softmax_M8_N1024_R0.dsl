...
// Llama-2-7B style single-head attention-score softmax tile:
// logical shape [B=1, H=1, M=8, N=1024], head_dim=128.
// Each row computes softmax(qk_row / sqrt(head_dim)) across all 1024 columns.
__attribute__((always_inline)) inline [aicore] void attention_softmax_row1024_simd_ub(
    ...
    __VEC_SCOPE__ {
        ...
        for (uint16_t blk = 0; blk < 16; ++blk) {
            ...
            vmuls(vec_scaled, vec_x, 0.08838834764831845, pat_all_b32);
            vcmax(vec_blk_max, vec_scaled, pat_all_b32, MODE_ZEROING);
        ...
        for (uint16_t blk = 0; blk < 16; ++blk) {
            vlds(vec_x, memA, 64 * blk, NORM);
            vmuls(vec_scaled, vec_x, 0.08838834764831845, pat_all_b32);
            vsub(vec_shift, vec_scaled, vec_row_max, pat_all_b32);
