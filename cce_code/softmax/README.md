...
- 数据目录：`macro_instr_ir_layout/abcabc`
- VfSim 基础提交：`9099a26`
- 调度策略：当前平衡预留策略，向后观察窗口为 8
- CAModel 周期直接统计自目录内已有日志，未重新运行 CAModel
- 该目录只包含 U2/U4；U1 与原始布局相同，因此不重复列出

| 指令组合 | Unroll | CAModel | VfSim | 误差 | 精度 |
|---|---:|---:|---:|---:|---:|
| `vexpdif + vmulscvt` | U2 | 579 | 631 | +52 | 91.02% |
| `vexpdif + vmulscvt` | U4 | 593 | 632 | +39 | 93.42% |
| `vexpdif + vcvt` | U2 | 630 | 618 | -12 | 98.10% |
| `vexpdif + vcvt` | U4 | 617 | 622 | +5 | 99.19% |
| `vsub + vexp + vmulscvt` | U2 | 690 | 685 | -5 | 99.28% |
| `vsub + vexp + vmulscvt` | U4 | 696 | 689 | -7 | 98.99% |
| `vsub + vexp + vcvt` | U2 | 676 | 669 | -7 | 98.96% |
| `vsub + vexp + vcvt` | U4 | 682 | 677 | -5 | 99.27% |

ABCABC 布局 8 个 case 的平均预测精度为 **97.28%**。
