...
当前对外保留两个输入对象，但预测入口只有一条 Canonical 主线：

- `api.frontend.CanonicalVfInfo`：版本化的目标输入契约。validator 不修改输入；Python 容器和 C++ 对象本身不承诺深度不可变。
- `api.vf_info.VFInfo`：迁移期兼容输入。预测前必须经 `ValueVersioningPass` 转换为 `CanonicalVfInfo`，Core 不再直接消费它。
...
现有 frontend 入口以 canonical 为主：

- `InputAPI.load_legacy_json_vf_info(path)`：显式把旧 JSON trace 加载为迁移期 `VFInfo`。
...
- `InputAPI.load_cce_vf_info(path, kernel_name=None, loop_params=None)`：显式返回迁移期 `VFInfo`；新调用方应使用 canonical 入口。
...
- `InputAPI.to_canonical(vf_info)`：严格版本化完整的逻辑 IR，不修补 legacy 缺失 operand。
- `InputAPI.adapt_legacy_vf_info(vf_info)`：在 legacy 边界修补历史格式允许省略的 scalar operand，再执行版本化。
- `InputAPI.load_json_trace()`、`InputAPI.load_cce_file()` 和 `CoreVfCostModel.predict_vf_cycles()` 仅为带 `DeprecationWarning` 的源码兼容包装。
...
- `VFInfoLowerer().lower(vf_info)`：兼容 API 名称；内部固定执行 `ValueVersioningPass -> CoreLoweringPass`，不再运行旧 `V0/mem0` lowering。
...
- `CoreVfCostModel().predict_legacy_vf_cycles(vf_info)`：显式适配旧 `VFInfo` 后复用 canonical 主线。
...
- loop 保留结构化 `count/unroll/body`、induction variable 和 entry/back-edge/exit 关系；validator 检查三类 definition 的作用域和类型一致性。Membar 名称必须非空；当前只有 `VST_VLD` 和 `VLD_VST` 具有时序语义，其它类型进入 ControlUnit 后记录 `unsupported_membar_type` warning。
...
C++ canonical 预测主入口位于 `native/SimulatorRunner.h`。`CanonicalProgramLowering` 使用 definition ID、每层动态 iteration path 和 loop-carried binding 生成动态指令，贯穿 `staticInstructionId`、`streamSeq` 及 value last-use/keep 标记，并应用 canonical 中已知的 `UarchConfig` override。该入口不调用旧 `lowerVfInfoValueIds()`、`normalizeProgramVregLiveRanges()` 或 `canonicalizeSingleSuperIterationLoops()`；`runLegacyVfInfo()` 通过 `LegacyVfInfoAdapter` 版本化后复用 `runCanonicalVfInfo()`，`runVfInfo()` 仅为弃用包装。含 Membar 的 innermost unroll 会保守回退为 1，并记录 `membar_unroll_disabled`。

canonical `uarch` override 的字段类型和适用目标由 `configs/uarch_override_schema.json` 统一定义。Python 直接读取该 schema，C++ 只读表由 `tools/generate_uarch_override_schema_cpp.py` 生成；已登记的 integer、boolean、string 字段不执行隐式类型转换。`canonical_dynamic_instruction_limit` 是 Python IFU 迁移期保护，因此明确标记为 Python-only；C++ 对 Python-only 或未知字段返回结构化错误，不会校验通过后静默忽略。native 测试双向比较 schema 的 C++ 字段集合与实际 resolver 字段集合。
...
`core/` 后端仍消费 JSON-like payload，但所有公共预测入口都先形成 `CanonicalVfInfo`，再通过 `CoreLoweringPass` 接入。旧 JSON 和旧 `VFInfo` 只作为边界兼容格式，不再拥有独立 Core lowering/normalization 路径。
...
当前 canonical last-use 标注仍会在第一次取指时预展开动态流。直接 Canonical 输入的 `uarch.canonical_dynamic_instruction_limit` 默认限制为 20000 条，超过时在分配无界内存前明确报错；这是迁移期保护，不是最终的流式 lifetime 实现。legacy adapter 为保持历史行为会显式写入 `canonical_dynamic_instruction_limit=0`，Core 只读取配置，不根据 `source.adapter` 改变语义。调用方仍可在旧输入中显式设置非零上限。
